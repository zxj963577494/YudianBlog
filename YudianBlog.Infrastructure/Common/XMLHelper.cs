...
namespace YudianBlog.Infrastructure.Common
{
    public class XMLHelper
    {
        ...
        public DataView GetDataViewByXml(string strWhere, string strSort, string TableName)
        {
            try
            {
                ...
                ds.ReadXml(filename);
                DataView dv = new DataView(ds.Tables[TableName]); //创建DataView来完成排序或筛选操作    
                if (strSort != null)
                {
        ...
        /// XMLProsess.Insert(path, "/Node","Value")
        /// XMLProsess.Insert(path, "/Node","Value")
        public static void UpdateNameSpace(string path, string node, string value)
        {
            try
            {
                XmlDocument doc = XMLLoad(path);
                ...
                XmlNode xn = doc.SelectSingleNode("descendant::" + node, xnm);
                xn.InnerText = value;
                doc.Save(AppDomain.CurrentDomain.BaseDirectory.ToString() + path);
            }
            catch { }
